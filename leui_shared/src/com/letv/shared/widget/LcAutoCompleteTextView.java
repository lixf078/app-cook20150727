 ...
 */
public class LcAutoCompleteTextView extends EditText implements Filter.FilterListener {
    ...
    private LcListPopupWindow mPopup;
    ...
    final static InputMethodReflector HIDDEN_METHOD_REFLECTOR = new InputMethodReflector();

    public LcAutoCompleteTextView(Context context) {
        this(context, null);
    }
    
    public LcAutoCompleteTextView(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.autoCompleteTextViewStyle);
    }
    
    public LcAutoCompleteTextView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        
        mPopup = new LcListPopupWindow(context, attrs, defStyle);
        mPopup.setBlurAfterView(this);
        mPopup.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
        mPopup.setPromptPosition(LcListPopupWindow.POSITION_PROMPT_BELOW);

        TypedArray a = context.obtainStyledAttributes(
                attrs, R.styleable.LcAutoCompleteTextView, R.attr.Leui_AutoCompleteTextViewStyle, 0);

        mThreshold = a.getInt(
        		R.styleable.LcAutoCompleteTextView_android_completionThreshold, 2);

        mPopup.setListSelector(a.getDrawable(R.styleable.LcAutoCompleteTextView_android_dropDownSelector));
        mPopup.setVerticalOffset((int)
                a.getDimension(R.styleable.LcAutoCompleteTextView_android_dropDownVerticalOffset, 0.0f));
        mPopup.setHorizontalOffset((int)
                a.getDimension(R.styleable.LcAutoCompleteTextView_android_dropDownHorizontalOffset, 0.0f));

        // Get the anchor's id now, but the view won't be ready, so wait to actually get the
        // view and store it in mDropDownAnchorView lazily in getDropDownAnchorView later.
        // Defaults to NO_ID, in which case the getDropDownAnchorView method will simply return
        // this TextView, as a default anchoring point.
        mDropDownAnchorId = a.getResourceId(R.styleable.LcAutoCompleteTextView_android_dropDownAnchor,
                View.NO_ID);
        
        // For dropdown width, the developer can specify a specific width, or MATCH_PARENT
        // (for full screen width) or WRAP_CONTENT (to match the width of the anchored view).
        mPopup.setWidth(a.getLayoutDimension(
                R.styleable.LcAutoCompleteTextView_android_dropDownWidth,
                ViewGroup.LayoutParams.WRAP_CONTENT));
        mPopup.setHeight(a.getLayoutDimension(
                R.styleable.LcAutoCompleteTextView_lcDropDownHeight,
                ViewGroup.LayoutParams.WRAP_CONTENT));
        
        mHintResource = a.getResourceId(R.styleable.LcAutoCompleteTextView_android_completionHintView,
                android.R.layout.simple_dropdown_item_1line);
        
        mPopup.setOnItemClickListener(new DropDownItemClickListener());
        setCompletionHint(a.getText(R.styleable.LcAutoCompleteTextView_android_completionHint));
    ...
    public void setPopupListDivider(Drawable divider) {
        mPopup.setItemDivider(divider);
    }

    public void setPopupBackground(Drawable d) {
        mPopup.setBackgroundDrawable(d);
    }

    public void setPopupBlurRadius(int radius) {
        mPopup.setBlurRadius(radius);
    }
    ...
    /**
     * Private hook into the on click event, dispatched from {@link com.letv.shared.widget.LcAutoCompleteTextView.PassThroughClickListener}
     */
    ...
    void doAfterTextChanged() {
        ...
        if (enoughToFilter()) {
            if (mFilter != null) {
                ...
                performFiltering(getText().toString(), mLastKeyCode);
            }
        } else {
    ...
    private void performCompletion(View selectedView, int position, long id) {
        if (isPopupShowing()) {
            ...
            if (mItemClickListener != null) {
                final LcListPopupWindow list = mPopup;
    ...
    public void dismissDropDown() {
        InputMethodManager imm = HIDDEN_METHOD_REFLECTOR.peekInstance();
        if (imm != null) {
    ...
    public void ensureImeVisible(boolean visible) {
        mPopup.setInputMethodMode(visible
                ? LcListPopupWindow.INPUT_METHOD_NEEDED : LcListPopupWindow.INPUT_METHOD_NOT_NEEDED);
    ...
    public boolean isInputMethodNotNeeded() {
        return mPopup.getInputMethodMode() == LcListPopupWindow.INPUT_METHOD_NOT_NEEDED;
    }
    ...
    public void showDropDown() {
        ...
        if (!isPopupShowing()) {
            ...
            mPopup.setInputMethodMode(LcListPopupWindow.INPUT_METHOD_NEEDED);
     ...
     * @see com.letv.shared.widget.LcAutoCompleteTextView#setOnDismissListener(com.letv.shared.widget.LcAutoCompleteTextView.OnDismissListener)
     */
    ...
    private static class InputMethodReflector {
    	private Method peekInstance;
    	
    	public InputMethodReflector() {
    		try {
				peekInstance = InputMethodManager.class.getDeclaredMethod("peekInstance");
    	...
    	InputMethodManager peekInstance() {
    		try {
				return (InputMethodManager) peekInstance.invoke(null);
			} catch (Exception e) {
				e.printStackTrace();
			}
