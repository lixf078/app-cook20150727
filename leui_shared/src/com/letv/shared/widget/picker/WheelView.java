...
//            Log.d("test","scrollingOffset:  "+scrollingOffset+" distance "+distance+"   scrollLimit  "+scrollLimit);
            if(isVertical&&scrollLimit!=0){//in vertical mode, judge whether to scroll
                if(Math.abs(scrollingOffset)<scrollLimit&&Math.abs(scrollingOffset+distance)>=scrollLimit){
//                    Log.d("test","prestop...");
                    int lastOffset = scrollingOffset;
                    if(distance>=0){
                        doScroll(scrollLimit-lastOffset);
                    }else {
                        doScroll(-scrollLimit-lastOffset);
                    }

                    return;
                }else if(Math.abs(scrollingOffset)>=scrollLimit){
//                    Log.d("test","stop...");
