...
public class DateWheel extends LinearLayout{
    ...
    public void setCalendar(Calendar calendar, boolean islunar, boolean istimed) {
        ...
        if(istimed) {
            ...
            // [+LEUI] [BUG][MOBILEP-7978] [wangziming] fix setCalendar no-work.
        } else {
            this.mCalendar = this.mTimedDate = calendar;
            // [-LEUI] [BUG][MOBILEP-7978] [wangziming] end
        }
...
//        dayView.setVisibleItems(7);
    ...
    public void updateDate(int year, int month, int dayOfMonth) {
        if (year > MAX_YEAR || year < MIN_YEAR)
            return;
        yearIndex = year - MIN_YEAR;
        yearView.setCurrentItem(yearIndex);
        monthIndex = month - 1;
        monthView.setCurrentItem(monthIndex);
        modifyDayView(year, month);
        dayIndex = getDayIndex(year, month, dayOfMonth);
        dayView.setCurrentItem(dayIndex);
    }

    /**
     * get the day's index of list
     * @param year
     * @param month
     * @param dayOfMonth
     */
    private int getDayIndex(int year, int month, int dayOfMonth) {
    ...
    public void onUpdateDate() {
        ...
        if(!isLunar) {
            ...
            if (mLeapMonth >= 1 && mLeapMonth <= 12) {
                if (monthIndex < mLeapMonth)
                    solarInfo = LunarCalendar.lunarToSolar(yearIndex + MIN_YEAR, monthIndex + 1, dayIndex + 1,
                                    isLeapMonth);
                else
