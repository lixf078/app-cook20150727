...
import com.letv.shared.widget.pulltorefresh.internal.FlipLoadingLayout;
import com.letv.shared.widget.pulltorefresh.internal.LoadingLayout;
import com.letv.shared.widget.pulltorefresh.internal.RotateLoadingLayout;
import com.letv.shared.widget.pulltorefresh.internal.ViewCompat;
import com.letv.shared.widget.pulltorefresh.internal.RefreshCompletedListener;
import com.letv.shared.widget.pulltorefresh.internal.LeRotateLoadingLayout;
import com.letv.shared.widget.pulltorefresh.internal.LeFooterRotateLoadingLayout;

import java.lang.String;
import java.lang.System;

public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T>,RefreshCompletedListener  {

    // ===========================================================
    // Constants
    // ===========================================================
    public static final float START_REFRESHING_OFFSET = 0.3f;
    public static final float ANIMATION_ENTER_OFFSET = 0.3f;
    ...
    public static final int SMOOTH_SCROLL_LONG_DURATION_MS = /*325*/600;
    ...
    protected int mRefreshableViewLayout = -1;
    ...
    public LoadingLayout getmHeaderLayout() {
    ...
    public void forceReset(){
        ...
        if( ((LeRotateLoadingLayout)mHeaderLayout).getAnimator() != null) {
            ((LeRotateLoadingLayout)mHeaderLayout).getAnimator().removeAllListeners();
        }
        ((LeRotateLoadingLayout)mHeaderLayout).cancelAnimator=false;
        ((LeRotateLoadingLayout)mHeaderLayout).resetImageHeader();
        ((LeFooterRotateLoadingLayout)mFooterLayout).cancelAnimator=false;
    }

    @Override
    public void onRefreshComplete() {
        if(isRefreshing()){
            if (((LeRotateLoadingLayout)mHeaderLayout).cancelAnimator==false){
                ((LeRotateLoadingLayout)mHeaderLayout).cancelAnimator = true;
            }
...
//    }
    ...
    /**
     * @return Either {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.Orientation#VERTICAL} or
     *         {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.Orientation#HORIZONTAL} depending on the scroll direction.
     */
     ...
     * {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.State#PULL_TO_REFRESH} state.
     */
    ...
    /**
     * Called when the UI has been to be updated to be in the
     * {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.State#REFRESHING} or {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.State#MANUAL_REFRESHING} state.
     * 
     * @param doScroll - Whether the UI should scroll for this event.
     */
    protected void onRefreshing(final boolean doScroll) {
        if (mMode.showHeaderLoadingLayout()) {
            if(mHeaderLayout instanceof  LeRotateLoadingLayout){
                ((LeRotateLoadingLayout)mHeaderLayout).forceRefreshingImpl();
            }else{
    ...
    /**
     * Called when the UI has been to be updated to be in the
     * {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.State#RELEASE_TO_REFRESH} state.
     */
    ...
    /**
     * Called when the UI has been to be updated to be in the
     * {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.State#RESET} state.
     */
    ...
    private void addRefreshableView(Context context, T refreshableView) {
        if (this instanceof PullToRefreshAdapterViewLayout && refreshableView instanceof FrameLayout) {
            mRefreshableViewWrapper =  (FrameLayout) refreshableView;
        } else {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_lePtrMode)) {
            mMode = Mode.mapIntToValue(a.getInteger(R.styleable.PullToRefresh_lePtrMode, 0));
        }

        if (a.hasValue(R.styleable.PullToRefresh_lePtrAnimationStyle)) {
            mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
                    R.styleable.PullToRefresh_lePtrAnimationStyle, 0));
        }
        
        if (a.hasValue(R.styleable.PullToRefresh_lePtrRefreshableViewLayout)) {
            mRefreshableViewLayout = a.getResourceId(R.styleable.PullToRefresh_lePtrRefreshableViewLayout, -1);
        }
        ...
        ((LeRotateLoadingLayout)mHeaderLayout).setRefreshCompletedListener(this);

        mFooterLayout = createLoadingLayout(context, Mode.PULL_FROM_END, a,false);
        ((LeFooterRotateLoadingLayout)mFooterLayout).setRefreshCompletedListener(this);
...
//        mFooterLayout = createLoadingLayout(context, Mode.PULL_FROM_END, a);
         ...
         * Styleables from XML
         */
        if (a.hasValue(R.styleable.PullToRefresh_lePtrRefreshableViewBackground)) {
            Drawable background = a.getDrawable(R.styleable.PullToRefresh_lePtrRefreshableViewBackground);
            if (null != background) {
                mRefreshableView.setBackground(background);
            }
        }

        if (a.hasValue(R.styleable.PullToRefresh_lePtrOverScroll)) {
            mOverScrollEnabled = a.getBoolean(R.styleable.PullToRefresh_lePtrOverScroll, true);
        }

        if (a.hasValue(R.styleable.PullToRefresh_lePtrScrollingWhileRefreshingEnabled)) {
            mScrollingWhileRefreshingEnabled = a.getBoolean(
                    R.styleable.PullToRefresh_lePtrScrollingWhileRefreshingEnabled, false);
        }
    ...
    private void pullEvent() {
        ...
        if (newScrollValue != 0 && !isRefreshing()) {
            ...
            float scale = getScale(Math.abs(newScrollValue)/(float)itemDimension);//Math.abs(newScrollValue) / (float) itemDimension;
            ...
            if (mState != State.PULL_TO_REFRESH && (1+ START_REFRESHING_OFFSET)*itemDimension >= Math.abs(newScrollValue)) {

                setState(State.PULL_TO_REFRESH);
            } else if (mState == State.PULL_TO_REFRESH && (1+ START_REFRESHING_OFFSET)*itemDimension < Math.abs(newScrollValue)) {
    ...
    private float getScale(float x) {
        if(x<=(1-ANIMATION_ENTER_OFFSET)){
            return 0;
        }else if(x<=1+ START_REFRESHING_OFFSET){
            return (1+ START_REFRESHING_OFFSET)/(START_REFRESHING_OFFSET +ANIMATION_ENTER_OFFSET)*(x-(1-ANIMATION_ENTER_OFFSET));
        }else {
            return x;
        }
        ...
        LoadingLayout createLoadingLayout(Context context, Mode mode, Orientation scrollDirection, TypedArray attrs,
            ...
            if (!isHeader){
                switch (this) {
                    case ROTATE:
                    default:
                        return new LeFooterRotateLoadingLayout(context, mode, scrollDirection, attrs);
                    case FLIP:
                        return new FlipLoadingLayout(context, mode, scrollDirection, attrs);
                }
//                return createLoadingLayout(context, mode, scrollDirection, attrs);
                ...
                switch (this) {
                    ...
                    default:
                        ...
                        return new LeRotateLoadingLayout(context,mode,scrollDirection,attrs);
                    case FLIP:
                        return new FlipLoadingLayout(context, mode, scrollDirection, attrs);
                }
            }
    ...
    public static enum Mode {
         ...
         * {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase#setRefreshing() setRefreshing()}.
         */
     ...
     * {@link com.letv.shared.widget.pulltorefresh.PullToRefreshAdapterViewBase#setOnLastItemVisibleListener}.
     * 
     * @author Chris Banes
     */
     ...
     * (such as sounds). See (
     * {@link com.letv.shared.widget.pulltorefresh.PullToRefreshAdapterViewBase#setOnPullEventListener}.
     * 
     * @author Chris Banes
     */
    public static interface OnPullEventListener<V extends View> {
         ...
         * @param direction - One of {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.Mode#PULL_FROM_START} or
         *            {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.Mode#PULL_FROM_END} depending on which direction
         *            the user is pulling. Only useful when <var>state</var> is
         *            {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.State#PULL_TO_REFRESH} or
         *            {@link com.letv.shared.widget.pulltorefresh.PullToRefreshBase.State#RELEASE_TO_REFRESH}.
         */
    ...
    final class SmoothScrollRunnable implements Runnable {
        ...
        public void run() {
            ...
            // Call OnPullEventListener
            if (null != mOnPullEventListener && 0 == mCurrentY) {
                mOnPullEventListener.onPullEvent(PullToRefreshBase.this, State.SMOOTHSCROLLFINISHED, mCurrentMode);
            }
        }
