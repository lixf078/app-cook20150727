...
public class LeTabWidgetUtils {
    ...
    public static void setTabWidgetLayout(Context context, LinearLayout bottomView, boolean hasTitle, boolean isDialerApp) {
        ...
        switch (count) {
            case 1:
                // According to designer's suggestion, scale touch area if there is only one tab
                end = (screenWidth - (int)(tabWidth * SCALE_TAB_WIDTH_IF_1_TAB_ONLY)) / 2;
                break;
            case 2:
        ...
        for (int i=0; i<count; i++) {
            ...
            // According to designer's suggestion, scale touch area if there is only one tab
            if (count == 1) {
                params.width = (int)(tabWidth * SCALE_TAB_WIDTH_IF_1_TAB_ONLY);
            } else {
