...
public class LeRadioButton extends RadioButton implements LeCheckable, ValueAnimator.AnimatorUpdateListener,
    ...
    public LeRadioButton(Context context, AttributeSet attrs) {
        this(context, attrs, android.R.attr.checkedTextViewStyle);
    }

    public LeRadioButton(Context context, AttributeSet attrs, int defStyle) {
        ...
        if(theme.resolveAttribute(android.R.attr.colorControlActivated, outValue, true)) {
            mBoxTrackColorOn = outValue.data;
            mArrowColorWithoutBorder = outValue.data;
        }
        
        if(theme.resolveAttribute(android.R.attr.colorControlNormal, outValue, true)) {
...
/*            for (int i = 0; i < n; i ++) {
            ...
            mWithoutBoxBorder = a.getBoolean(R.styleable.leCheckbox_leBoxWithoutBorder, mWithoutBoxBorder);
            if (mWithoutBoxBorder) {
                mBoxSize = res.getDimensionPixelSize(R.dimen.le_default_box_size);
            }
            mBoxInnerPadding = a.getDimensionPixelSize(R.styleable.leCheckbox_leBoxInnerPadding, mBoxInnerPadding);
            if (a.hasValue(R.styleable.leCheckbox_leTextOnColor)) {
                mTextColorOnChecked = a.getColorStateList(R.styleable.leCheckbox_leTextOnColor);
            }
            mBoxTrackColorOn = a.getColor(R.styleable.leCheckbox_leBoxOnColor, mBoxTrackColorOn);
            mArrowColor = a.getColor(R.styleable.leCheckbox_leBoxArrowColor, mArrowColor);
            mArrowColorWithoutBorder = a.getColor(R.styleable.leCheckbox_leBoxArrowColorWithoutBorder, mArrowColorWithoutBorder);
            mBoxSize = a.getDimensionPixelSize(R.styleable.leCheckbox_leBoxSize, mBoxSize);
            mIsBoxTextOnRight = a.getBoolean(R.styleable.leCheckbox_leBoxIsTextOnRight, mIsBoxTextOnRight);
        }
        a.recycle();
...
//        clickable = a.getBoolean(R.styleable.View_clickable, clickable);
        ...
        if (mWithoutBoxBorder) {
            ...
            ObjectAnimator animatorOutEnlarge = ObjectAnimator.ofInt(
                    this, "DynimacRadius", 0, mMaxCircleRadius);
            ObjectAnimator animatorArrowShown = ObjectAnimator.ofFloat(
                    this, "ArrowInterpolatedTime", 0, 1);
            ...
            ObjectAnimator animatorArrowHidden = ObjectAnimator.ofFloat(
                    this, "ArrowInterpolatedTime", 1, 0);
            ObjectAnimator animatorInNarrow = ObjectAnimator.ofInt(this, "DynimacRadius",
                    mMaxCircleRadius, 0);
    ...
    public void setBoxInnerPadding(int padding) {
        mBoxInnerPadding = padding;
    }
    
    public int getBoxInnerPadding() {
        return mBoxInnerPadding;
    }
    
    public void setTrackBoxColor(int colorOn) {
    ...
    public void setArrowInterpolatedTime(float interpolatedTime) {
        ...
        if (mAnimateTextView != null) {
            mAnimateTextViewColor = (Integer) mArgbEvaluator.evaluate(
                    interpolatedTime,
                    mAnimateTextColorOrigin, mAnimateTextColorOnChecked);
        }
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        int boxLeft = isBoxOnRight() ?
                getWidth() - paddingRight - mBoxSize - mMeasureBorderSize :
                mMeasureBorderSize + padidngLeft;
        int boxRight = isBoxOnRight() ?
                getWidth() - paddingRight - mMeasureBorderSize :
                mBoxSize + mMeasureBorderSize + paddingRight;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mShowAnimatior != null && mHiddenAnimatior != null) {
            ...
            canvas.translate(left + getScrollX(), top + getScrollY());
            if (!mWithoutBoxBorder) {
                mCirclePath.reset();
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(LeRadioButton.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(LeRadioButton.class.getName());
    }
}
