	...
	public void run() {
		...
		while (true) {
			try {
				...
				if (!request.readCacheWithoutTimeLimit()) {
					// If it is completely expired, just send it to the
					// network.
					if (entry.isExpired()) {
				...
				NetworkResponse netWorkResponse =  new NetworkResponse(entry.data,
						entry.responseHeaders);
				netWorkResponse.setFromCache(true);//自己添加的
				...
				if (!entry.refreshNeeded()
						|| request.readCacheWithoutTimeLimit()) {
					...
					// response,
					// but we need to also send the request to the network
					// for
					// refreshing.
					...
					// Post the intermediate response back to the user and
					// have
