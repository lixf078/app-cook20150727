...
public class RoundedDrawable extends Drawable {
	...
	private float mCornerRadius = 0;// X,Y轴的 巨型角度
	...
	// 我自己添加的，为了能够在X轴和Y轴上设置不同的 圆角弧度
	// private float mXCornerRadius = 0;
	// private float mYCornerRadius = 0;
	private CornerLocation mCornerLocation = CornerLocation.ALL;

	public enum CornerLocation {
		LEFT, RIGHT, TOP, BOM, ALL
	}

	private RectF mDrawableRect_tmp = new RectF();
	...
	private void updateShaderMatrix() {
		...
		// --  我自己添加的两个分支，判断是否只是顶部或者底部画圆角
		if (mCornerLocation == CornerLocation.TOP) {
			mDrawableRect_tmp.set(mDrawableRect.left, mDrawableRect.top,
					mDrawableRect.right, mDrawableRect.bottom + mCornerRadius);

		} else if (mCornerLocation == CornerLocation.BOM) {

			mDrawableRect_tmp.set(mDrawableRect.left, mDrawableRect.top
					- mCornerRadius, mDrawableRect.right, mDrawableRect.bottom);

		} else {
			mDrawableRect_tmp.set(mDrawableRect);
		}
	}
	...
	public void draw(Canvas canvas) {
		...
		if (mOval) {
			...
			if (mBorderWidth > 0) {
				...
				canvas.drawRoundRect(mDrawableRect_tmp, mCornerRadius,
	...
	public RoundedDrawable setCornerRadius(float radius, CornerLocation location) {
		mCornerRadius = radius;
		this.mCornerLocation = location;
		return this;
	}
	...
	// ----我自己添加的
	// public RoundedDrawable setXCornerRadius(float radius) {
	// mXCornerRadius = radius;
	// return this;
	// }
	//
	// public RoundedDrawable setYCornerRadius(float radius) {
